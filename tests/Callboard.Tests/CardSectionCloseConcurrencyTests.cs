        ...
        try
        {
            ...
            foreach (var (round, outcome) in results)
            {
                var closed = outcome.Match(
                    ...
                    onOpenObligations: o => throw new Xunit.Sdk.XunitException($"round {round}: unexpected OpenObligations({o.SectionId})"),
                    onOpenUndeferredQuestion: q => throw new Xunit.Sdk.XunitException($"round {round}: unexpected OpenUndeferredQuestion({q.QuestionId})"),
                    onUnresolvedAddressedThread: t => throw new Xunit.Sdk.XunitException($"round {round}: unexpected UnresolvedAddressedThread({t.CardId})"),
                    onBlockedByOpenProductOwnerQuestion: b => throw new Xunit.Sdk.XunitException($"round {round}: unexpected BlockedByOpenProductOwnerQuestion({b.BlockId})"),
