...
public sealed class CardCommentImmutabilityTests
{
    ...
    public void CardStore_EntireStaticMethodSurface_IsExplicitlyAccountedFor()
    {
        ...
        var expectedMembers = new[]
        {
            ...
            "IsHazardCard",                          // §10 block A: the IsRegisterCard counterpart narrowed to CardKind.Hazard, shared by WorkingContextAssembler's live-register scan; never touches a CardFile's Comments
            ...
            "OwningSectionId",                       // §9 remediation round three (F2): pure accessor answering which section an arbitrary card belongs to, over CardFrontmatter.Kind/Id/Section, shared by every site that means "the owning section of an arbitrary card" (comment promote, nit disposition raise, finding blind-spot raise); never touches a CardFile's Comments
            "PromoteComment",                       // §9 remediation, round two (S4): allocates the raised card's identity, pre-creates its directory, then delegates to AcquireLocksAndRecord/PromoteCommentUnderLocks — never touches an existing card's Comments except appending the one resolving comment
            "PromoteCommentUnderLocks",             // §9 remediation, round two (S4): writes the raised card create-only via AtomicWrite, then appends exactly one resolving comment to the original card via `card with { Comments = [.. card.Comments, comment] }`, rolling the raised card back (by content, not by path) on the original's own write failing — never an edit or drop of an existing comment
            ...
            "RollbackRaisedCommentCard",             // §9 remediation, round two (S4): the same compare-then-delete rollback applied to PromoteComment's own raised card — never touches an existing card at all
