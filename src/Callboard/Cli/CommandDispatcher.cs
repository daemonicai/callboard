...
internal static class CommandDispatcher
{
    ...
    internal abstract record ParsedCommand
    {
        ...
        /// <summary>
        /// <c>block amendment-requested</c> (work-lifecycle: "`amendment-requested` is the
        /// architect deliberately reopening an approved block" — the only route from
        /// <c>approved</c> back to work that is not a supervisor's recurrence, since an approval
        /// certifies one exact state and any change to that state spends it, requiring a fresh
        /// review rather than a re-assertion of existing claims). The one door to that edge —
        /// <c>block transition ... amendment-requested</c> refuses outright during parse (Architect
        /// ruling, same one-door discipline <see cref="BlockApprove"/> already established).
        /// <see cref="Id"/> is resolved through <see cref="Cards.CardIdentityResolver"/> at execute
        /// time, the same identity-addressing convention that verb already uses.
        /// </summary>
    ...
    /// <summary>
    /// <c>block amendment-requested</c> (work-lifecycle: "`amendment-requested` is the architect
    /// deliberately reopening an approved block" — the only route from <c>approved</c> back to
    /// work that is not a supervisor's recurrence). Resolves <c>--id</c> through
    /// <see cref="ResolveCardReference"/> the same way <see cref="RunBlockApprove"/> does, then
    /// delegates to <see cref="Cards.CardStore.RecordAmendmentRequest"/>. Result reuses <see cref="
