...
internal static class CommandDispatcher
{
    ...
    internal abstract record ParsedCommand
    {
        ...
        Func<RegisterDischarge, TResult> onRegisterDischarge, Func<DecisionSupersede, TResult> onDecisionSupersede, Func<ChangeArchive, TResult> onChangeArchive, Func<RulePromote, TResult> onRulePromote, Func<RuleAuthor, TResult> onRuleAuthor, Func<RuleCompact, TResult> onRuleCompact, Func<RuleProposeCompact, TResult> onRuleProposeCompact, Func<RulePromoteConstitution, TResult> onRulePromoteConstitution,
            Func<BlockApprove, TResult> onBlockApprove, Func<NitRaise, TResult> onNitRaise, Func<NitDisposition, TResult> onNitDisposition, Func<Recertify, TResult> onRecertify, Func<AmendmentRequested, TResult> onAmendmentRequested);
        ...
        /// <summary>
        /// <c>block recertify</c> (§8 block C, review-certification: "Recertification re-asserts an
        /// existing claim set" / "Recertification is bounded"). <see cref="Id"/> is resolved through
        /// <see cref="Cards.CardIdentityResolver"/> at execute time, the same identity-addressing
        /// convention <see cref="BlockApprove"/> already established.
        /// </summary>
        /// <param name="State">The exact amended state this recertification re-certifies against,
        /// including any uncommitted working-tree content it covers — caller-supplied text,
        /// verified against nothing, the same convention <see cref="BlockApprove.ReviewedState"/>
        /// uses. Stamped as the card's new <c>reviewed_state</c> only when every named claim is
        /// asserted; discarded otherwise (Architect ruling: on a refusal, <c>reviewed_state</c> is
        /// left exactly as it was).</param>
        /// <param name="AssertedClaimIds">The current approval's claim ids this call asserts, each
        /// checked non-blank during parse. Together with <see cref="RefusedClaimIds"/> must, at
        /// execute time, name every claim the current approval carries and no other — checked
        /// against the card's own record, not here.</param>
        ...
        /// <summary>
        /// <c>block amendment-requested</c> (§8 block C remediation, work-lifecycle:
        /// "`amendment-requested` is the architect deliberately reopening an approved block for a
        /// further amendment"). The one door to that edge — <c>block transition ... amendment-
        /// requested</c> refuses outright during parse (Architect ruling, same one-door discipline
        /// <see cref="BlockApprove"/> and <see cref="Recertify"/> already established).
        /// <see cref="Id"/> is resolved through <see cref="Cards.CardIdentityResolver"/> at execute
        /// time, the same identity-addressing convention those two verbs already use.
        /// </summary>
        internal sealed record AmendmentRequested(
            string Id, CardOwner ActingRole, string? ChangeName, string WorkingDirectory, DateTimeOffset Timestamp) : ParsedCommand
        {
            internal override TResult Match<TResult>(Func<Version, TResult> onVersion, Func<IndexRebuild, TResult> onIndexRebuild, Func<BlockTransition, TResult> onBlockTransition, Func<BlockGate, TResult> onBlockGate, Func<BlockAddBlocker, TResult> onBlockAddBlocker, Func<BlockRemoveBlocker, TResult> onBlockRemoveBlocker, Func<SectionVerdict, TResult> onSectionVerdict, Func<SectionClose, TResult> onSectionClose, Func<SectionStatus, TResult> onSectionStatus, Func<FindingRecord, TResult> onFindingRecord, Func<FindingStatus, TResult> onFindingStatus, Func<RuleCreate, TResult> onRuleCreate, Func<HazardCreate, TResult> onHazardCreate, Func<ObligationCreate, TResult> onObligationCreate, Func<DecisionCreate, TResult> onDecisionCreate, Func<SectionCreate, TResult> onSectionCreate, Func<QuestionCreate, TResult> onQuestionCreate, Func<RegisterDischarge, TResult> onRegisterDischarge, Func<DecisionSupersede, TResult> onDecisionSupersede, Func<ChangeArchive, TResult> onChangeArchive, Func<RulePromote, TResult> onRulePromote, Func<RuleAuthor, TResult> onRuleAuthor, Func<RuleCompact, TResult> onRuleCompact, Func<RuleProposeCompact, TResult> onRuleProposeCompact, Func<RulePromoteConstitution, TResult> onRulePromoteConstitution, Func<BlockApprove, TResult> onBlockApprove, Func<NitRaise, TResult> onNitRaise, Func<NitDisposition, TResult> onNitDisposition, Func<Recertify, TResult> onRecertify, Func<AmendmentRequested, TResult> onAmendmentRequested) =>
                onAmendmentRequested(this);
        }
    }
        ...
        try
        {
            ...
            var outcome = parseResult.Match(
                onReady: ready => ready.Command.Match(
                    ...
                    onRecertify: parsed => RunBlockRecertify(parsed, resolvedLockTimeout),
                    onAmendmentRequested: parsed => RunBlockAmendmentRequested(parsed, resolvedLockTimeout),
    ...
    /// (review-certification: "Approval is role-bounded" — 8.13's approval half; block C is expected
    /// to reach recertification's role check the same way). <see langword="private"/>:
    ...
    /// <summary>
    /// <c>block recertify</c> (§8 block C, review-certification: "Recertification re-asserts an
    /// existing claim set" / "Recertification is bounded"). Resolves <c>--id</c> through
    /// <see cref="ResolveCardReference"/> the same way <see cref="RunBlockApprove"/> does, then
    /// delegates to <see cref="Cards.CardStore.RecordRecertification"/>. Role-bounded the same as
    /// approval (review-certification: "Approval is role-bounded" — 8.13's recertification half,
    /// reusing <see cref="Cards.CardStore.IsApprovingRole"/> rather than a second predicate).
    /// <see langword="private"/>: <see cref="CommandParser"/> cannot name this method.
    /// </summary>
    private static CommandOutcome RunBlockRecertify(ParsedCommand.Recertify parsed, TimeSpan lockTimeout)
    {
        var repoRoot = RepoRootResolver.Resolve(parsed.WorkingDirectory);
        if (repoRoot is null)
        {
        ...
        return outcome.Match<CommandOutcome>(
            ...
            {
                FilePath = resolved.FilePath!,
                Id = recertified.Card.Frontmatter.Id,
                // Never null here: RecordApprovalUnderExistingLock stamps reviewed_state on every
                // approval (CommandParser.ParseBlockApprove refuses an empty/whitespace --state
                // before that call is ever reached), and RecordRecertificationUnderExistingLock
                // refuses unless the card's current state is already Approved — so a card that
                // reaches this success case necessarily already carries one.
                ReviewedState = recertified.Card.BlockFields.ReviewedState!,
                AssertedClaimIds = recertified.AssertedClaimIds,
                RefusedClaimIds = [],
                Transitioned = false,
                Round = recertified.Card.BlockFields.Round,
                ActingRole = parsed.ActingRole.ToWireString(),
                Timestamp = parsed.Timestamp,
                Notice = RecertificationNotice,
            }),
            onClaimsRefused: claimsRefused => new CommandOutcome.Success(new BlockRecertifyResult
            {
                FilePath = resolved.FilePath!,
                Id = claimsRefused.Card.Frontmatter.Id,
                ReviewedState = claimsRefused.Card.BlockFields.ReviewedState!,
                AssertedClaimIds = claimsRefused.AssertedClaimIds,
                RefusedClaimIds = claimsRefused.RefusedClaimIds,
                Transitioned = true,
                Round = claimsRefused.Card.BlockFields.Round,
                ActingRole = parsed.ActingRole.ToWireString(),
                Timestamp = parsed.Timestamp,
                Notice = RecertificationNotice,
            }),
            onRoleNotPermitted: roleNotPermitted => RoleNotPermitted(
                "recording a recertification", roleNotPermitted.AttemptedRole, [CardOwner.Reviewer, CardOwner.Supervisor]),
            onNotABlockCard: notABlock => WrongCardKind(resolved.FilePath!, CardKind.Block, notABlock.Kind, "'block recertify' only applies to a block card"),
            onCardNotFound: notFound => new CommandOutcome.Refusal(
                "card-not-found", $"no card file exists at '{notFound.FilePath}' to recertify."),
            onNotApproved: notApproved => new CommandOutcome.Refusal(
                "not-currently-approved",
                $"'{resolved.FilePath}' cannot be recertified — it is currently '{notApproved.CurrentState.ToWireString()}', not 'approved'."),
            onAlreadyRecertified: _ => new CommandOutcome.Refusal(
                "recertification-already-performed",
                $"'{resolved.FilePath}' has already been recertified once against its current approval — " +
                "review-certification permits at most one recertification per approval; a further amendment " +
                "requires a new round."),
            onUnknownClaimIds: unknownClaimIds => new CommandOutcome.Refusal(
                "unknown-claim-id",
                $"'--assert'/'--refuse' named claim id(s) the current approval does not carry: " +
                $"{string.Join(", ", unknownClaimIds.ClaimIds)}."),
            onMissingClaimOutcomes: missing => new CommandOutcome.Refusal(
    ...
    /// <c>block amendment-requested</c> (§8 block C remediation, work-lifecycle:
    /// "`amendment-requested` is the architect deliberately reopening an approved block for a
    /// further amendment"). Resolves <c>--id</c> through <see cref="ResolveCardReference"/> the
    /// same way <see cref="RunBlockApprove"/>/<see cref="RunBlockRecertify"/> do, then delegates to
    /// <see cref="Cards.CardStore.RecordAmendmentRequest"/>. Result reuses <see cref="
    /// BlockTransitionResult"/> — this verb's success is exactly that shape (an edge applied, who
    /// applied it and when, the block's resulting state), so it does not need its own result type.
    /// <see langword="private"/>: <see cref="CommandParser"/> cannot name this method.
    /// </summary>
    private static CommandOutcome RunBlockAmendmentRequested(ParsedCommand.AmendmentRequested parsed, TimeSpan lockTimeout)
    {
        var repoRoot = RepoRootResolver.Resolve(parsed.WorkingDirectory);
        if (repoRoot is null)
        {
        ...
        var outcome = CardStore.RecordAmendmentRequest(
            repoRoot, resolved.FilePath!, parsed.ActingRole, parsed.Timestamp, lockTimeout, parsed.ChangeName);

        return outcome.Match<CommandOutcome>(
            onRequested: requested => new CommandOutcome.Success(new BlockTransitionResult
            {
                FilePath = resolved.FilePath!,
                Transition = "amendment-requested",
                From = BlockFlowState.Approved.ToWireString(),
                To = BlockFlowState.Briefed.ToWireString(),
                ActingRole = parsed.ActingRole.ToWireString(),
                Timestamp = parsed.Timestamp,
                Base = requested.Card.BlockFields.Base,
                Round = requested.Card.BlockFields.Round,
            }),
            onRoleNotPermitted: roleNotPermitted => RoleNotPermitted(
                "requesting an amendment", roleNotPermitted.AttemptedRole, CardOwner.Architect),
            onNotABlockCard: notABlock => WrongCardKind(resolved.FilePath!, CardKind.Block, notABlock.Kind, "'block amendment-requested' only applies to a block card"),
            onCardNotFound: notFound => new CommandOutcome.Refusal(
                "card-not-found", $"no card file exists at '{notFound.FilePath}' to reopen."),
            onUndefinedTransition: undefined => new CommandOutcome.Refusal(
                "undefined-transition",
