...
internal static class CardKindWireFormat
{
    ...
    /// rule, hazard, decision, in <see cref="RecognisedValues"/>'s own order. §12 block B's
    /// register area reads this to name one lane per register kind, rather than hand-listing the
    /// four a second time. A fixed literal, not filtered from <c>ByWireValue.Values</c>: <see
    /// cref="Dictionary{TKey,TValue}"/> enumeration order is an implementation detail, not a
    /// contract, and this order is one a caller renders by.</summary>
